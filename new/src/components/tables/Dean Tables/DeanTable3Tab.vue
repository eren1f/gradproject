          ...
          <div class="shadow overflow-hidden rounded-lg">
            <table class="w-full md:min-w-full">
              ...
              <tbody class="bg-white divide-y divide-blue-200">
                ...
                <tr v-else>
                  <td class="px-2 py-2 text-sm text-gray-500 text-center" colspan="7">
...
const filteredAndSortedStudents = computed(() => {
  ...
  if (query) {
    filteredRequests = filteredRequests.filter(request => {
      if (radioButtonSelection.value === '1') {
        ...
        const departmentName = request.getStudentDepartment().toLowerCase();
        return departmentName.includes(query) || 
               departmentName.split(' ').reverse().join(' ').includes(query);
      }
      return false;
    });
  }
  ...
  const sortedRequests = filteredRequests.sort((a, b) => {
    ...
    }else if(currentSort.value.column === 'byDep')
            {
